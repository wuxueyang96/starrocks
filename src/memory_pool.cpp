...
void LayeredMemoryPool::init_small_blocks() {
    // 按比例分配每层的空间
    // 小块使用频率更高，分配更多空间给小块层
    constexpr std::array<float, NUM_LAYERS> layer_ratios = {
        0.25f, 0.20f, 0.15f, 0.12f, 0.10f, 0.08f, 0.06f, 0.04f
    };
    ...
    while (current) {
        if (!current->in_use && current->size >= total_size) {
            ...
            if (remaining >= sizeof(LargeBlockHeader) + MIN_BLOCK_SIZE) {
                LargeBlockHeader* new_block = reinterpret_cast<LargeBlockHeader*>(
                    reinterpret_cast<uint8_t*>(current) + total_size);
...
void LayeredMemoryPool::deallocate_aligned(void* ptr) {
    ...
    // 读取元数据
    AlignedMetadata* metadata = reinterpret_cast<AlignedMetadata*>(
        static_cast<uint8_t*>(ptr) - sizeof(AlignedMetadata));
...
LayeredMemoryPool::LargeBlockHeader* LayeredMemoryPool::get_large_block_header(void* ptr) const {
    ...
    return reinterpret_cast<LargeBlockHeader*>(
        reinterpret_cast<uint8_t*>(ptr) - sizeof(LargeBlockHeader));
}
