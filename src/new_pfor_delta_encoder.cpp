...
std::vector<uint8_t> NewPForDeltaEncoder::encodeImpl(const std::vector<uint32_t>& values) {
    ...
    // Encode exception values using Simple9 (NEW: more efficient than VarInt)
...
std::vector<uint32_t> NewPForDeltaEncoder::decodeImpl(const std::vector<uint8_t>& encoded) {
    ...
    if (offset >= encoded.size()) {
        throw std::runtime_error("Invalid NewPForDelta encoding: missing bits_per_value");
    }
