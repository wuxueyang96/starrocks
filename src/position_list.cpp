...
void PositionList::decode(const std::vector<uint8_t>& encoded_data, EncodingType encoding_type) {
    ...
    if (encoded_data.size() > 0 && encoded_data[0] == 0xFF) {
        ...
        for (uint32_t i = 0; i < num_blocks; ++i) {
            ...
            std::vector<uint8_t> block_data(encoded_data.begin() + offset,
                                           encoded_data.begin() + offset + block_size);
            offset += block_size;
...
std::vector<uint8_t> PositionList::encodeWithBlocks(EncodingType encoding_type,
    ...
    for (size_t block_idx = 0; block_idx < num_blocks; ++block_idx) {
        ...
        // Extract block
        std::vector<uint32_t> block(sorted_positions.begin() + start_idx,
                                   sorted_positions.begin() + end_idx);
