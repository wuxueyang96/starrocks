...
Status BuiltinInvertedReader::create(const std::shared_ptr<TabletIndex>& tablet_index, LogicalType field_type,
    ...
    if (is_string_type(field_type)) {
        ...
        const auto omit_term_freq_and_position = get_omit_term_freq_and_position(tablet_index->index_properties());
...
Status BuiltinInvertedReader::load(const IndexReadOptions& opt, void* meta) {
    ...
    const BitmapIndexPB bitmap_index_meta = static_cast<BuiltinInvertedIndexPB*>(meta)->bitmap_index();
