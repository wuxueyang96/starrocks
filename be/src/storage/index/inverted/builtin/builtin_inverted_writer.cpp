...
Status BuiltinInvertedWriter::create(const TypeInfoPtr& typeinfo, TabletIndex* tablet_index,
    ...
    bool omit_freq_and_position = get_omit_term_freq_and_position(tablet_index->index_properties());

    std::unique_ptr<BitmapIndexWriter> writer;
    RETURN_IF_ERROR(BitmapIndexWriter::create(typeinfo, &writer, gram_num, !omit_freq_and_position));
